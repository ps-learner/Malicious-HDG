...
ip_to_asn = asn.dropna(subset=["asn"]).set_index("ip")["asn"].to_dict()
    ...
    for d in group["domain"]:
        ...
        if d in dns_lookup.index:
            ...
            for ip in a_recs + aaaa_recs:
                if ip in ip_id:
                    resolves_to.append({"src": did, "dst": ip_id[ip]})
                    a = ip_to_asn.get(ip)
                    if a in asn_id:
                        belongs_asn.append({"src": ip_id[ip], "dst": asn_id[a]})
            for ns in ns_recs:
                if ns in ns_id:
                    shares_ns.append({"src": did, "dst": ns_id[ns]})
        if d in whois_lookup.index:
            r = whois_lookup.loc[d, "registrar"]
            if pd.notna(r) and r in reg_id:
                registered_by.append({"src": did, "dst": reg_id[r]})
